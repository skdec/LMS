...
const PaymentModal = ({ invoice, isOpen, onClose, onPaymentAdded }) => {
  ...
  useEffect(() => {
    if (invoice && isOpen) {
      fetchPaymentHistory();
      setLatestPayment(null); // Reset to hide preview when modal opens or invoice changes
    }
  }, [invoice, isOpen]);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      onPaymentAdded();
      fetchPaymentHistory();
      // Set latest payment for receipt preview
      setLatestPayment({
  ...
  const handleSendMail = async () => {
    ...
    setMailStatus("");
    try {
      await axiosInstance.post(`/invoice/${invoice._id}/send-email`);
      setMailStatus("Invoice sent to student email!");
    } catch (err) {
      setMailStatus("Failed to send invoice. Try again.");
    } finally {
      setMailLoading(false);
    }
  };

  const handleSendMailForPayment = async (payment, paymentKey) => {
    setMailLoadingId(paymentKey);
    setMailStatusId((prev) => ({ ...prev, [paymentKey]: "" }));
    try {
      await axiosInstance.post(`/invoice/${invoice._id}/send-email`, {
        payment,
      });
      setMailStatusId((prev) => ({
        ...prev,
        [paymentKey]: "Receipt sent to student email!",
      }));
    } catch (err) {
      setMailStatusId((prev) => ({
        ...prev,
        [paymentKey]: "Failed to send receipt. Try again.",
      }));
    } finally {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="bg-white rounded-xl p-6 w-full max-w-5xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center space-x-2">
            ...
            >
              ...
              <span>{mailLoading ? "Sending..." : "Send to Mail"}</span>
            </button>
            <button
              onClick={onClose}
          ...
          {mailStatus && (
            <div
              className={`mt-2 text-sm ${
                mailStatus.includes("sent") ? "text-green-600" : "text-red-600"
              }`}
            >
              {mailStatus}
            </div>
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      {monthData.payments.map((payment, pIndex) => {
                        const paymentKey = `${monthData.month}-${pIndex}`;
                        return (
                          <div
                            key={pIndex}
                          ...
                          >
                            ...
                            <div className="text-right">
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleSendMailForPayment(payment, paymentKey)
                                }
                                disabled={mailLoadingId === paymentKey}
                              >
                                {mailLoadingId === paymentKey
                                  ? "Sending..."
                                  : "Send to Mail"}
                              </button>
                              {mailStatusId[paymentKey] && (
                                <div
                                  className={`mt-1 text-xs ${
                                    mailStatusId[paymentKey].includes("sent")
                                      ? "text-green-600"
                                      : "text-red-600"
                                  }`}
                                >
        ...
        {/* Show receipt preview for selected payment */}
        ...
        {/* Hidden ReceiptPreview for printing */}
        <div style={{ display: "none" }}>
          <div ref={receiptRef}>
            <ReceiptPreview
              invoice={invoice}
              student={invoice.studentId}
              payment={selectedHistoryPayment || latestPayment}
            />
          </div>
        </div>
      </div>
